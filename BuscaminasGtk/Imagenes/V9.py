...
class Tablero:
    def __init__(self):
        ...
        self.imagenes.append(gtk.gdk.pixbuf_new_from_file("xcelda_3.png"))  #Por ahora, 3 o mas equivalen a esta imagen
        self.imagenes.append(gtk.gdk.pixbuf_new_from_file("xcelda_4.png"))  #Por ahora, 3 o mas equivalen a esta imagen
        self.imagenes.append(gtk.gdk.pixbuf_new_from_file("xcelda_5.png"))  #Por ahora, 3 o mas equivalen a esta imagen
        self.imagenes.append(gtk.gdk.pixbuf_new_from_file("xcelda_6.png"))  #Por ahora, 3 o mas equivalen a esta imagen
        ...
        for i in range(self.filas):
            for j in range(self.columnas):
                if i>-1 and j>-1 and i<self.filas+2 and j<self.columnas+2:
                    ...
                    if j%2==0:
                        espacio=1
                        FFixed.put(self.button,(i+espacio)*15,j*15)
                    else:
                        FFixed.put(self.button,i*15,j*15)
                    if mina==1:
            ...
            if event.button==1:
                ...
                if j%2==0:
                    if i>0 and j>0 and i<self.filas-1 and j<self.columnas-1 and total==0 and turno==4:
                        self.FinMina(widget,event,lista,oculta,i+1,j-1,4)
                        turno=5
                    if i>0 and j>0 and i<self.filas-1 and j<self.columnas-1 and total==0 and turno==5:
                        self.FinMina(widget,event,lista,oculta,i-1,j-1,5)
                else:
                    if i>0 and j>0 and i<self.filas-1 and j<self.columnas-1 and total==0 and turno==4:
                        self.FinMina(widget,event,lista,oculta,i+1,j+1,4)
                        turno=5
    ...
    def CrearTablero(self,widget,filas,columnas,minas):
        ...
        for i in range(filas):
            ...
            for j in range(columnas):
                ...
                self.button.unset_flags(gtk.CAN_FOCUS)
                self.button.set_can_focus(False)
                self.button.get_image().set_from_pixbuf(self.imagenes[0])
                if j%2==0:
                    table.put(self.button,(i+espacio)*20,j*20)
                else:
                    table.put(self.button,i*20,j*20)
                self.button.show()
                if oculta[i][j]==1:
                    mina=1
                else:
        ...
        self.button=gtk.Button("Quit")
        self.button.connect("clicked",lambda w:gtk.main_quit())
        table.put(self.button,0,250)
        self.button.set_size_request(200,30)
        self.button.show()

        self.button=gtk.Button("Reset")
        self.button.connect("clicked",self.Restart)
        table.put(self.button,0,275)
        self.button.set_size_request(200,30)
        self.button.show()

        #table.put(self.etq_tpo,0,200)
        #self.timer.show()

        table.put(self.etq_tpo,0,225)
        self.etq_tpo.show()
