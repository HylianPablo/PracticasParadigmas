...
class Tablero:
    def __init__(self):
        ...
        self.imagenes.append(gtk.gdk.pixbuf_new_from_file("ycelda_3.png"))  
        self.imagenes.append(gtk.gdk.pixbuf_new_from_file("ycelda_4.png"))  
        self.imagenes.append(gtk.gdk.pixbuf_new_from_file("ycelda_5.png"))  
        self.imagenes.append(gtk.gdk.pixbuf_new_from_file("ycelda_6.png"))  #Por ahora, 6 o mas equivalen a esta imagen
    ...
    def MuestraTablero(self,oculta):
        ...
        FTable.attach(self.button,1,self.filas+2,self.columnas+2,self.columnas+3)
        self.button.show()

        self.button=gtk.Button("Reset")
        self.button.connect("clicked",self.Restart)
        FTable.attach(self.button,1,self.filas+2,self.columnas+3,self.columnas+4)
        self.button.show()
        FTable.show()
        self.window.show()
    ...
    def FinMina(self,widget,event,lista,oculta,i,j,turno):
        ...
        if self.disponible==True:
            if event.button==1:
                ...
                if total==0 and i>3 and j>3 and i<self.filas-1 and j<self.columnas-1 and turno==0:
                    self.FinMina(widget,event,lista,oculta,i-1,j,0)
                #Con los turnos se controla la recursividad    
                    turno=1
                if i>3 and j>3 and i<self.filas-1 and j<self.columnas-1 and total==0 and turno==1:
                    self.FinMina(widget,event,lista,oculta,i+1,j,1)
                    turno=2
                if i>3 and j>3 and i<self.filas-1 and j<self.columnas-1 and total==0 and turno==2:
                    self.FinMina(widget,event,lista,oculta,i,j+1,2)
                    turno=3
                if i>3 and j>3 and i<self.filas-1 and j<self.columnas-1 and total==0 and turno==3:
    ...
    def CrearTablero(self,widget,filas,columnas,minas):
        ...
        for i in range(filas+4):
            ...
            for j in range(columnas+4):
                if i>1 and j>1 and i<filas+2 and j<columnas+2: 
                    ...
                    lista[i].append(C)
                    self.button.connect("button-release-event",self.FinMina,lista,oculta,i,j-2,0)
        ...
        self.button=gtk.Button("Quit")
        self.button.connect("clicked",lambda w:gtk.main_quit())
        table.attach(self.button,0,filas+2,columnas+2,columnas+3)
        self.button.show()

        self.button=gtk.Button("Reset")
        self.button.connect("clicked",self.Restart)
        table.attach(self.button,0,filas+2,columnas+3,columnas+4)
        self.button.show()
