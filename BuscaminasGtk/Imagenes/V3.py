...
class Tablero:
    ...
    def FinMina(self,widget,event,lista,oculta,i,j):
        celda=lista[i][j]
        mina=celda.mina
        #i=celda.i
        #j=celda.j
        ...
        if self.disponible==True:
            if event.button==1:
                ...
                self.FinMina(widget,event,lista,oculta,i-1,j)
            elif event.button==3:
    ...
    def CrearTablero(self,widget,filas,columnas,minas):
        ...
        for i in range(filas*2):
            ...
            for j in range(columnas*2):
                if i>1 and j>1 and i<11 and j<11:
        ...
        for i in range(filas*2):
            ...
            for j in range(columnas*2):
                if i>1 and j>1 and i<11 and j<11: 
                    ...
                    table.attach(self.button,i+1,i+2,j,j+1)
                    self.button.show()
                    if oculta[i][j]==1:
                        mina=1
                    else:
                    ...
                    lista[i].append(C)
                    self.button.connect("button-release-event",self.FinMina,lista,oculta,i,j-2)


        self.button=gtk.Button("Quit")
        self.button.connect("clicked",lambda w:gtk.main_quit())
        table.attach(self.button,0,filas*2,12,13)
        self.button.show()

        self.button=gtk.Button("Reset")
        self.button.connect("clicked",self.Restart)
        table.attach(self.button,0,filas*2,13,14)
        self.button.show()
