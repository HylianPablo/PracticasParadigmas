...
class Tablero:
    def __init__(self):
        ...
        self.button.connect("clicked",self.CrearTablero,8,9,10)
        ...
        self.button.connect("clicked",self.CrearTablero,30,16,99)
    ...
    def LeerFichero(self,widget):
        ...
        '''for i in range(size1):
            self.OcultaFich.append([])
            for j in range(size2):
                self.OcultaFich[i].append([])'''
        ...
        for ii in range(size1):
            for jj in range(size2):
                self.OcultaFich[ii][jj]=minado[cuentaminas]
                cuentaminas+=1'''
    ...
    def MuestraTablero(self,lista):
        ...
        self.window.set_border_width(20)
        self.window.set_size_request((self.filas+8)*24,(self.columnas+8)*24)
        self.window.connect("delete_event",self.delete_event)   #Cierra el programa si se pulsa la cruz del marco
        self.window.set_position(gtk.WIN_POS_CENTER)
        ...
        FFixed.put(self.button,self.filas+100,(self.columnas-1)*24)
        ...
        FFixed.put(self.button,0,(self.columnas+5)*24)
        ...
        if self.disponible==True:
                ...
                if mina==1:
                    if self.PrimeraMina==True:
                        ...
                        if celda.marcada==True:
                            ...
                            self.Fixed.put(self.Avisos,self.filas*2,(self.columnas+3)*24)
                            self.Avisos.show()
                        else:
                            self.Avisos.hide()
            ...
            #### MARCADAS ####
                ...
                if celda.cerrada==True and celda.marcada==False and self.NumeroMarcadas<self.NumeroMinas:
                    ...
                    self.Fixed.put(self.Etq_marcadas1,(self.filas+75)*2,(self.columnas+2)*24)
    ...
    def CrearTablero(self,widget,filas,columnas,minas):
        ...
        self.window.set_border_width(20)
        self.window.set_size_request((filas+8)*24,(columnas+8)*24)   #Encontrar un valor que ajuste la ventana, por ahora, este con 9x9 esta bien
        self.window.set_position(gtk.WIN_POS_CENTER)
        ...
        if self.Fichero==False:
            ...
            for i in range(filas):
                for j in range(columnas):
                    oculta[i][j]=self.minado[counter]
                    counter+=1
        ...
        for i in range(filas):
            ...
            for j in range(columnas):
                ...
                if j%2!=0:
                    self.Fixed.put(self.button,(i+espacio)*20,j*20)
                else:
        ...
        self.button.connect("clicked",self.Restart)
        self.Fixed.put(self.button,0,(self.columnas+5)*24)
        self.button.set_size_request(200,30)
        self.button.show()
        ...
        self.button.get_image().set_from_pixbuf(self.smiley[0])
        self.button.connect("clicked",self.CrearTablero,self.filas,self.columnas,self.NumeroMinas)
        self.Fixed.put(self.button,self.filas+100,(self.columnas-1)*24)
        self.button.set_size_request(45,45)
        self.button.show()

        Etq_minas=gtk.Label()
        Etq_minas.set_markup("<b>Numero de minas: </b>")
        self.Fixed.put(Etq_minas,self.filas*2,(self.columnas+1)*24)
        Etq_minas.show()

        Etq_minas1=gtk.Label(self.NumeroMinas)
        self.Fixed.put(Etq_minas1,(self.filas+65)*2,(self.columnas+1)*24)
        Etq_minas1.show()

        self.Etq_marcadas=gtk.Label()
        self.Etq_marcadas.set_markup("<b>Numero de marcadas: </b>")
        self.Fixed.put(self.Etq_marcadas,self.filas*2,(self.columnas+2)*24)
        ...
        self.Fixed.put(self.etq_tpo,self.filas*1,(self.columnas-1)*24)       #Etiqueta del tiempo
